...
# Load YOLO model
yolo_model = YOLO("D:/project/myapp/src/Backend/best_100_epoch.pt")
...
faster_rcnn_model.load_state_dict(torch.load('D:/project/myapp/src/Backend/fasterrcnn_resnet50_epoch_100.pth', map_location=device), strict=False)
    ...
    for i in range(len(predictions[0]['boxes'])):
        ...
        if confidence > 0.5:
            ...
            detections.append({
                ...
                'bbox': bbox,
                'color': (0, 255, 0) if label_name == "bird" else (0, 0, 255)
            })
    return detections
...
def process_image(image_array):
    ...
    # Combine detections
    final_detections = faster_rcnn_detections if faster_rcnn_detections else yolo_detections
        ...
        if file_extension in ['.jpg', '.jpeg', '.png', '.webp']:
            ...
            try:
                ...
                for frame in frames:
                    ...
                    raw_detections = [
                        ([det['bbox'][0], det['bbox'][1], det['bbox'][2] - det['bbox'][0], det['bbox'][3] - det['bbox'][1]], det['confidence'], det['label']) 
                        for det in detections
                    ]

                    tracker_outputs = tracker.update_tracks(raw_detections, frame=frame)

                    frame_with_boxes = frame.copy()
                    draw_bounding_boxes(frame_with_boxes, detections, precision, accuracy)
                    processed_frames.append(frame_with_boxes)

                avg_precision = total_precision / len(frames)
                avg_accuracy = total_accuracy / len(frames)
